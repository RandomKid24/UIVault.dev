...
const SimulationRouter: React.FC<{ id: string }> = ({ id }) => {
  switch (id) {
    ...
    // Additional cases for consistency
    ...
    default: return (
      <div className="flex flex-col items-center gap-4 opacity-10">
        <Play size={24} strokeWidth={2} className="group-hover:text-cyan-500 transition-colors" />
        <span className="mono text-[8px] uppercase tracking-[0.6em] font-black">Hover_to_Test</span>
      </div>
    );
  }
};

const Lab: React.FC<LabProps> = ({ components, onSelect }) => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-12 pb-32">
      ...
      <aside className="w-full lg:w-80 lg:sticky lg:top-24 lg:h-[calc(100vh-160px)] space-y-10 overflow-y-auto pr-4 scrollbar-hide">
        ...
        {/* SIDEBAR SEARCH BAR */}
        <div className="p-6 bg-neutral-900/40 border border-white/10 rounded-[2rem] space-y-6 relative overflow-hidden group/searchbox">
          <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/5 to-transparent pointer-events-none" />
          
          <div className="flex items-center justify-between px-1 relative z-10">
            <div className="flex items-center gap-2">
               <Search size={12} className={isSearchFocused ? 'text-cyan-500 animate-pulse' : 'text-neutral-500'} />
               <span className="mono text-[9px] text-neutral-500 font-black uppercase tracking-[0.3em]">SEARCH_COMPONENTS</span>
            </div>
            ...
            <div className={`relative transition-all duration-700 rounded-2xl overflow-hidden border-2 ${
              ...
              <div className="flex items-center px-4 relative z-10">
                <input 
                  ...
                  className="w-full bg-transparent py-5 px-1 mono text-[11px] text-white focus:outline-none placeholder:text-neutral-700 uppercase tracking-[0.1em]"
                />
                {searchQuery && (
                  <motion.button 
              ...
              <div className="h-1 w-full bg-white/5 relative z-10 overflow-hidden">
                 <motion.div 
                   animate={{ 
                     width: `${Math.min(100, (searchQuery.length / 10) * 100)}%`,
          ...
          <div className="flex items-center justify-between px-1">
             ...
             <div className={`mono text-[7px] font-black uppercase tracking-tighter ${isNoResults ? 'text-rose-500' : 'text-neutral-700'}`}>
                {searchQuery ? (isNoResults ? 'STATE_VOID' : 'SYNC_ACTIVE') : 'IDLE'}
             </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2 mono text-[9px] text-neutral-500 uppercase font-black tracking-widest">
            <Filter size={10} /> COMPONENT_TYPES
          </div>
          <div className="flex flex-wrap lg:flex-col gap-2">
            {categories.map((cat) => (
              <motion.button
                ...
                whileHover={{ x: 5 }}
                className={`px-4 py-3 rounded-xl text-left mono text-[9px] font-black tracking-[0.2em] uppercase transition-all border-2 group relative overflow-hidden ${
        ...
        {/* QUICK LIST COMPONENT NAVIGATION */}
        <div className="space-y-4">
          <div className="flex items-center gap-2 mono text-[9px] text-neutral-500 uppercase font-black tracking-widest">
            <Hash size={10} /> ALL_COMPONENTS
          </div>
          <div className="flex flex-col gap-1 max-h-[350px] lg:max-h-none overflow-y-auto pr-2 scrollbar-hide custom-scrollbar">
            {filteredComponents.map((comp) => (
              ...
              >
                <span className="mono text-[10px] text-neutral-600 group-hover:text-white transition-colors truncate pr-4 uppercase">
                  {comp.name}
                </span>
      ...
      <div className="flex-1 space-y-16">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-white/5 pb-12">
          <div className="space-y-4">
            <h2 className="text-6xl lg:text-8xl font-black italic tracking-tighter uppercase leading-none">The_Lab</h2>
            <p className="text-neutral-500 mono text-[10px] tracking-[0.5em] uppercase">
              Experimental_Modules // {filteredComponents.length}_Primities_Calibrated
            </p>
          </div>
          <div className="flex items-center gap-4 px-6 py-3 bg-neutral-900 border border-white/5 rounded-full mono text-[9px] text-neutral-500 uppercase">
            <Eye size={14} className="text-cyan-500" /> Observation_Mode_Active
          </div>
        </header>
        ...
        <AnimatePresence mode="wait">
          {filteredComponents.length > 0 ? (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10 perspective-[2000px]"
            >
              {filteredComponents.map((comp, idx) => (
                ...
                >
                  <div className="absolute -inset-1 border border-cyan-500/0 group-hover:border-cyan-500/10 rounded-[2.6rem] transition-colors pointer-events-none z-0" />
                  
                  <div className="absolute top-4 left-4 w-4 h-4 border-t border-l border-white/10 group-hover:border-cyan-500/40 transition-colors z-20" />
                  <div className="absolute top-4 right-4 w-4 h-4 border-t border-r border-white/10 group-hover:border-cyan-500/40 transition-colors z-20" />
                  <div className="absolute bottom-4 left-4 w-4 h-4 border-b border-l border-white/10 group-hover:border-cyan-500/40 transition-colors z-20" />
                  <div className="absolute bottom-4 right-4 w-4 h-4 border-b border-r border-white/10 group-hover:border-cyan-500/40 transition-colors z-20" />

                  {/* Component Preview Chamber */}
                  <div className="h-60 bg-[#020202] relative flex items-center justify-center border-b border-white/10 rounded-t-[2.5rem] overflow-hidden group-hover:bg-black transition-colors">
                    <div className="absolute inset-0 pointer-events-none z-10 opacity-10 bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.25)_50%),linear-gradient(90deg,rgba(255,0,0,0.06),rgba(0,255,0,0.02),rgba(0,0,255,0.06))] [background-size:100%_2px,3px_100%]" />
                    <div className="absolute inset-0 opacity-[0.02] pointer-events-none [background-size:24px_24px] bg-[linear-gradient(white_1px,transparent_1px),linear-gradient(90deg,white_1px,transparent_1px)]" />
                  ...
                  <div className="p-8 flex flex-col gap-6 relative z-10 bg-[#080808] rounded-b-[2.5rem]">
                    <div className="space-y-1">
                      <div className="flex items-center gap-3 mb-2">
                        <span className="mono text-[8px] tracking-[0.2em] text-cyan-500 font-black uppercase px-2 py-0.5 border border-cyan-500/20 rounded-sm bg-cyan-500/5 transition-all group-hover:bg-cyan-500 group-hover:text-black">{comp.category}</span>
                        <div className="h-[1px] flex-1 bg-white/5" />
                      </div>
                      <h3 className="text-xl font-black italic tracking-tighter uppercase leading-tight group-hover:text-white transition-colors">{comp.name}</h3>
                      <p className="text-neutral-500 text-[11px] font-light leading-relaxed line-clamp-2 pt-2 group-hover:text-neutral-400 transition-colors">{comp.description}</p>
                    </div>

                    <div className="pt-6 border-t border-white/5 flex flex-col gap-5">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 mono text-[8px] text-neutral-700 font-black uppercase">
                          <Gauge size={10} /> Mass: {comp.difficulty}
                        </div>
                        <div className="flex items-center gap-1.5 mono text-[8px] text-neutral-800 font-bold">
                           <Activity size={10} /> ACTIVE_FIELD
                        </div>
                      </div>
                      ...
                      <div className="relative w-full h-10 group/btn overflow-hidden rounded-xl border border-white/5 bg-neutral-900/50 flex items-center justify-center">
                        <motion.div 
                          initial={false}
                          animate={{ x: hoveredId === comp.id ? 0 : -300 }}
                          className="absolute inset-0 bg-gradient-to-r from-cyan-600/10 via-cyan-500/10 to-transparent transition-all"
                        />
                        <span className="relative z-10 mono text-[8px] font-black text-neutral-600 group-hover:text-cyan-400 uppercase tracking-[0.3em] flex items-center gap-3">
                          EXPLORE_PRIMITIVE <ArrowUpRight size={10} />
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            >
              <div className="w-20 h-20 rounded-full bg-neutral-900 border border-white/5 flex items-center justify-center text-neutral-800 relative overflow-hidden">
                <Search size={32} strokeWidth={1} />
              </div>
              <div className="space-y-3">
                <h3 className="text-3xl font-black italic tracking-tighter uppercase text-white/40 leading-none">NO_MATCHES_DETECTED</h3>
                <p className="mono text-[10px] text-neutral-600 uppercase tracking-[0.4em] max-w-sm mx-auto leading-relaxed">
                  The search parameters did not return any functional modules.
                </p>
              </div>
              <motion.button 
                whileHover={{ scale: 1.05 }}
                onClick={() => { setSearchQuery(''); setSelectedCategory('all'); }}
                className="px-10 py-4 bg-white/5 border border-white/10 rounded-full mono text-[9px] font-black text-cyan-500 hover:bg-cyan-500 hover:text-black transition-all uppercase tracking-widest"
              >
                Clear_All_Filters
              </motion.button>
            </motion.div>
